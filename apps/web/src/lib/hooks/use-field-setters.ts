...
import { parseSystemPromptToSegments, segmentsToMarkdown } from "@chatterbox/prompt-assembly";
import { DEFAULT_SETTINGS } from "@/lib/defaults";
import type { Settings } from "@/lib/defaults";
import type { StructuredStoryState } from "@/lib/story-state-model";
import { parseMarkdownToStructured, structuredToMarkdown, reconcileEntities, remapEntityIds } from "@/lib/story-state-model";
...
function useStoryStateHandlers() {
  ...
  const [structuredState, setStructuredState] = useState<StructuredStoryState | null>(null);
  ...
  const handleStructuredStateUpdate = useCallback((updated: StructuredStoryState, onError?: () => void) => {
  ...
  const updateStoryStateFromSummary = useCallback(
    (newState: string) => {
      ...
      setStructuredState(prev => {
        ...
        const { entities, idRemap } = reconcileEntities(existingEntities, incoming.entities);
  ...
  return {
    ...
    structuredState, setStructuredState,
    handleStoryStateChange, handleStoryStateImport, handleStoryStateReset,
    handleStructuredStateUpdate, updateStoryStateFromSummary,
  };
}
...
export function useFieldSetters() {
  ...
  const [customSegments, setCustomSegments] = useState<SerializedSegment[] | null>(null);
  ...
  const handleSegmentUpdate = useCallback((segmentId: string, newContent: string) => {
    setCustomSegments(prev => {
      if (!prev) return prev;
      return prev.map(s =>
        s.id === segmentId
          ? { ...s, content: newContent, tokenEstimate: Math.ceil(newContent.length / 4) }
          : s,
      );
    });
  }, []);
  ...
  return {
    ...
    lastSummarizedTurn, setLastSummarizedTurn,
    lastPipelineTurn, setLastPipelineTurn,
