 ...
 *
 * Checks 1–4 from the plan: schema, preservation, novelty, completeness.
 * No LLM calls needed.
 */
...
export function validateState(
  ...
  // 2. Preservation check — all previous hard facts still present
  const prevHardFacts = extractHardFacts(previous);
  const allHardFactsPreserved = prevHardFacts.every(fact =>
    candidate.toLowerCase().includes(fact.toLowerCase())
  );
  ...
  const newHardFacts = candHardFacts.filter(
    fact => !prevHardFacts.some(pf => pf.toLowerCase() === fact.toLowerCase())
  );
  const factDetails = new Set(extractedFacts.map(f => f.detail.toLowerCase()));
  const noUnknownFacts = newHardFacts.every(
    fact => factDetails.has(fact.toLowerCase()) ||
      extractedFacts.some(ef => fact.toLowerCase().includes(ef.detail.toLowerCase().slice(0, 30)))
  );
