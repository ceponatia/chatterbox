...
export const MessageList = memo(function MessageList({
  ...
  const { visibleMessages, hiddenCount, handleShowMore } = useVisibleMessages(
    messages,
    topAnchorRef,
  );
...
function useVisibleMessages(
  ...
  if (visibleState.key !== conversationKey)
    setVisibleState({ key: conversationKey, count: INITIAL_VISIBLE });
  ...
  const handleShowMore = useCallback(() => {
    ...
    const scrollParent = anchor?.closest(
    ...
    requestAnimationFrame(() => {
      if (anchor && scrollParent)
        scrollParent.scrollTop += anchor.offsetTop - prevTop;
    });
  }, [topAnchorRef]);
